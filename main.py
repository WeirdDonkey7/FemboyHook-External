...
def discord_rpc_thread():
	presence = Presence(1368666172605730978)
	presence.connect()
	a = False
	while True:
		if discord_presence_enabled:
			if not a:
				try:
					presence.update(
						state="github.com/WeirdDonkey7/FemboyHook-External",
						details="Cs2 External",
						start=int(time.time()),
						large_image="do-i-look-cute-in-my-new-outfit-v0-rp2r8n7qvf1b1",
						large_text="Cs2 External Cheat",
						small_image="github",
						small_text="WeirdDonkey7 on GitHub",
						buttons=[{'label': 'Download', 'url': 'https://github.com/WeirdDonkey7/FemboyHook-External'}]
					)
				except Exception as e:
					pass
			a = True
...
def main():
	...
	pme.overlay_init(title="FemboyHook")
...
def GUI():
	...
	root = ctk.CTk()
	root.title("FemboyHook")
	root.iconbitmap("do-i-look-cute-in-my-new-outfit-v0-rp2r8n7qvf1b1.ico")
	...
	def open_github():
		open_url("https://github.com/WeirdDonkey7/FemboyHook-External")
...
if __name__ == "__main__":
	print("FFFF EEEE  M        M  BBBB    OOOO   Y       Y       H     H    OOOO     OOOO    K     K\nF    E     MMM    MMM  B   B  O    O   Y     Y        H     H   O    O   O    O   K  K\nFFF  EEEE  M   MM   M  BBBB   O    O     Y Y          HHHHHHH   O    O   O    O   KK\nF    E     M        M  B   B  O    O      Y           H     H   O    O   O    O   K  K\nF    EEEE  M        M  BBBB    OOOO       Y           H     H    OOOO     OOOO    K    K\n             \n\n                                                                      - By WeirdDonkey7 -\n")
	main()